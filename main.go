...
func main() {
	...
	if err = (&controllers.StsOperatorConfigReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
		Log:    ctrl.Log.WithName("controllers").WithName("StsOperatorConfig"),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "StsOperatorConfig")
		os.Exit(1)
	}
	//+kubebuilder:scaffold:builder

	createDefaultOperatorConfig(ctrl.GetConfigOrDie())
...
func createDefaultOperatorConfig(cfg *rest.Config) error {
	...
	config := &stsv1alpha1.StsOperatorConfig{}
	config.Namespace = defaultNamespace
	config.Name = defaultName

	err = c.Get(context.TODO(), types.NamespacedName{
		Name: defaultName, Namespace: defaultNamespace}, config)
	...
	if err != nil {
		if errors.IsNotFound(err) {
			...
			if err != nil {
				fmt.Println(fmt.Printf("Couldn't create OperatorConfig: %v", err))
				return err
			}
		}
